...
CONF.PATH.DATA = "/NetworkDocker/Data/Open3DSG_trainset"  # root for your custom dataset(s)
...
CONF.PATH.MYSET_CHECKPOINTS  = os.path.join(CONF.PATH.MYSET_ROOT, "open3dsg/output/checkpoints")
CONF.PATH.MYSET_FEATURES_OUT = os.path.join(CONF.PATH.BASE,  "open3dsg/output/features/myset")
CONF.PATH.MYSET_PREPROC_OUT  = os.path.join(CONF.PATH.BASE,  "open3dsg/output/preprocessed/myset")
CONF.PATH.MYSET_GRAPHS_OUT   = os.path.join(CONF.PATH.BASE,  "open3dsg/output/graphs/myset")
...
for key, path_str in CONF.PATH.items():
    ...
    is_output = any(tag in key for tag in ["OUT", "CHECKPOINT", "FEATURE", "PREPROC", "GRAPHS", "DATA_OUT"])
    if is_output:
    ...
    if any(ds.upper() in key for ds in USED_DATASETS) or is_myset:
        if not os.path.exists(path_str):
            raise FileNotFoundError(f"{key}: {path_str} does not exist (and is required).")
    else:
