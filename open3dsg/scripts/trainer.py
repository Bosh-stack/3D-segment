...
class D3SSGModule(lightning.LightningModule):
    ...
    def setup(self, stage: str):
        ...
        SCANNET_TRAIN = load_scan(
            CONF.PATH.SCANNET, "subgraphs/relationships_train.json"
        )
        ...
        if stage == "fit":
            ...
            if (
                not self.hparams["clean_pointnet"]
                and not self.model.rgb
                and not self.model.nrm
            ):
        ...
        if (
            not self.hparams["test"]
            and type(self.logger) == lightning.loggers.MLFlowLogger
        ):
            for hp_key, hp_val in self.hparams.items():
                if hp_key.startswith("Device"):
                    ...
                    self.logger.experiment.log_metric(
                        run_id=self.logger.run_id, key="param_" + hp_key, value=hp_val
                    )
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.Adam(
            self.model.parameters(), self.hparams["lr"], weight_decay=1e-5
        )

        if self.hparams["lr_scheduler"] == "cyclic":
            lr_scheduler = torch.optim.lr_scheduler.CyclicLR(
                optimizer,
                base_lr=self.hparams["lr"] - (self.hparams["lr"] / 2),
            ...
            lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
                ...
                mode="min",
                factor=0.5,
                patience=5 if not self.hparams["mini_dataset"] else 10000,
                threshold=0.0001,
                threshold_mode="rel",
            )
    ...
    def on_train_epoch_start(self) -> None:
        ...
        if type(self.logger) == lightning.loggers.MLFlowLogger:
            self.logger.experiment.log_metric(
                run_id=self.logger.run_id, key="current_epoch", value=self.current_epoch
            )
    ...
    def training_step(self, data_dict, *_, **__):
        ...
        if self.hparams.get("dump_features"):
            self._dump_features(
                data_dict, data_dict["objects_id"].size(0), path=self.clip_path
            )
            return
        ...
        for k, v in logs.items():
            self.log(
                k, v, sync_dist=True, batch_size=self.hparams["batch_size"]
            )  # , prog_bar=True if k=="train/loss" else False)
    ...
    def validation_step(self, data_dict, batch_ixd, dataloader_idx=0):
        ...
        for k, v in loss_dict.items():
            self.log(
                k, v, on_epoch=True, sync_dist=True, batch_size=1
            )  # , prog_bar=True if k=="val/loss" else False)
    ...
    def on_test_epoch_start(self) -> None:
        ...
        self.rel_vocab = F.normalize(
            torch.from_numpy(self.rel_mapper.encode(self.pred_class_dict)), dim=-1
        ).cuda()
        self.vis_dump_dir = (
            CONF.PATH.BASE
            + "/vis_graphs/"
    ...
    def test_step(self, data_dict, batch_ixd):
        ...
        if self.hparams.get("dump_features"):
            self._dump_features(
                pred_dict, data_dict["objects_id"].size(0), path=self.clip_path
            )
            return
        ...
        if self.hparams["predict_from_2d"]:
            (
                objects_predict_clip,
                objects_probs_clip,
                object_mostlikely_clip,
                objects_valid_clip,
            ) = self._predict_obj_from_clip(
                data_dict,
                data_dict["objects_id"].shape[0],
                query_classes=self.obj_class_dict,
                from_distill=False,
            )
            if self.hparams.get("blip"):
                (
                    ...
                    predicates_clip,
                    relationships_clip,
                    predicates_mapped_clip,
                    predicates_mapped_probs_clip,
                ) = self._predict_rel_from_blip(
                    data_dict,
                    object_mostlikely_clip,
                    objects_valid_clip,
                    data_dict["objects_id"].shape[0],
        ...
        if self.hparams.get("predict_materials"):
            (
                materials_predict,
                materials_probs,
                materials_mostlikely,
                materials_valid,
            ) = self._predict_obj_from_emb(
                pred_dict, data_dict["objects_id"].shape[0], query_materials
            )

        if self.hparams.get("predict_colors"):
            colors_predict, colors_probs, colors_mostlikely, colors_valid = (
                self._predict_obj_from_emb(
        ...
        if vis:
            ...
            objects_gt = [
                objects[0][
                    (data_dict["edges"] == i).cpu()[0][
                        : data_dict["predicate_count"][0]
                    ]
                ][0]
                for i in torch.unique(data_dict["edges"][0])
            ]
            self._vis_clip_graphs(
                data_dict,
                object_mostlikely,
                objects_gt,
                predicates,
    ...
    def _compute_loss(self, data_dict, mat_diff_loss_scale=0.001):
        ...
        if not self.hparams.get("pointnet2", False):
            ...
            data_dict["L_mat_diff"] = torch.FloatTensor([0]).to(
                data_dict["objects_id"].device
            )
    ...
    def _mask_features(
        self, data_dict, clip_obj_emb, clip_rel_emb, bidx, obj_count, rel_count
    ):
        obj_valids = None
        ...
        if type(clip_rel_emb) is torch.Tensor:
            ...
            clip_rel_emb_masked[clip_rel2frame_mask > 0] = clip_rel_emb[
                clip_rel2frame_mask > 0
            ]
    ...
    def _distillation_loss(self, batch_size, data_dict):
        ...
        for bidx in range(batch_size):
            ...
            if self.hparams["blip"] or self.hparams["llava"]:
                if self.hparams["avg_blip_emb"] or self.hparams["avg_llava_emb"]:
                    ...
                    if self.hparams["blip"]:
                        nan_mask = (
                            ~torch.isnan(clip_rel_emb_masked).all(dim=-1).all(dim=-1)
                        )
                    elif self.hparams["llava"]:
                        clip_rel_emb_masked = clip_rel_emb_masked.view(
                            clip_rel_emb_masked.shape[0], -1
                        )
                        nan_mask = ~torch.isnan(clip_rel_emb_masked).all(dim=-1)
            else:
            ...
            if (~obj_valids).all():
                ...
                obj_loss_i = (
                    1 - self.cosSim1(gcn_obj_emb[obj_valids], clip_obj_emb[obj_valids])
                ).mean()
            if (~nan_mask).all():
                ...
                rel_loss_i = (
                    1
                    - self.cosSim1(
                        gcn_rel_emb[nan_mask].view(rels, -1),
                        clip_rel_emb_masked[nan_mask].view(rels, -1),
                    )
                ).mean()
        ...
        data_dict["loss"] = (
            self.hparams.get("w_obj", 1.0) * data_dict["obj_loss"]
            + self.hparams.get("w_rel", 1.0) * data_dict["rel_loss"]
        )
    ...
    def _supervised_dist_loss(self, batch_size, data_dict):
        ...
        for bidx in range(batch_size):
            ...
            if not self.hparams.get("load_features", None):
                obj_valids, clip_obj_emb, _ = self._mask_features(
                    data_dict, clip_obj_emb, None, bidx, obj_count, None
                )
            else:
                obj_valids = data_dict["clip_obj_valids"][bidx][:obj_count]
            ...
            rel_loss_i = self.rel_supervision(
                rel_prediction, rel_classes.to(torch.float32)
            )
    ...
    def _dump_features(self, data_dict, batch_size, path=CONF.PATH.FEATURES):
        for bidx in range(batch_size):
            ...
            obj_clip_model = (
                ...
                if self.hparams["node_model"]
                and self.hparams["clip_model"] != "OpenSeg"
            ...
            if self.val_dataset is not None:
                obj_path = os.path.join(
                    path,
                    "export_obj_clip_emb_clip_"
                    + obj_clip_model.replace("/", "-")
                    + "_Topk_"
                    + str(self.hparams["top_k_frames"])
                    + "_scales_"
                    + str(self.hparams["scales"])
                    + "_vis_crit_"
                    + str(self.val_dataset.obj_vis_crit)
                    + "_vis_crit_mask_"
                    + str(self.val_dataset.obj_mask_crit),
                )
                rel_path = os.path.join(
                    path,
                    "export_rel_clip_emb_clip_"
                    + rel_clip_model.replace("/", "-")
                    + "_Topk_"
                    + str(self.hparams["top_k_frames"])
                    + "_scales_"
                    + str(self.hparams["scales"])
                    + "_vis_crit_"
                    + str(self.val_dataset.rel_vis_crit),
                )
            else:
                obj_path = os.path.join(
    ...
    @torch.no_grad()
    def _predict_obj_from_clip(
        self, data_dict, batch_size, query_classes=None, from_distill=True
    ):
        objs_predict = []
        objects_probs = []
        objs_mostlikly = []
        ...
        for bidx in range(batch_size):
            ...
            if from_distill:
                ...
                clip_obj_emb[torch.isnan(clip_obj_emb)] = graph_obj_emb[
                    torch.isnan(clip_obj_emb)
                ]
                clip_obj_emb = (
                    self.hparams.get("weight_2d", 0.5) * clip_obj_emb
                    + (1 - self.hparams.get("weight_2d", 0.5)) * graph_obj_emb
                )

            _, objs_probs = self.model.predict_nodes(
                clip_obj_emb.float(), query_classes
            )
    ...
    @torch.no_grad()
    def _predict_rel_from_blip(
        self, data_dict, obj_predict, obj_valids, batch_size, from_distill=True
    ):
        relationships = []
        predicates = []
        predicates_mapped = []
        ...
        for bidx in range(batch_size):
            ...
            if self.hparams.get("gt_objects"):
                object_edges = (
                    data_dict["objects_id"][bidx][:obj_count][edges]
                    .cpu()
                    .to(torch.long)
                )
            else:
            ...
            if from_distill:
                ...
                if self.hparams.get("avg_blip_emb"):
                    graph_rel_emb = graph_rel_emb.unsqueeze(-2).repeat(
                        1, clip_rel_emb.shape[-2], 1
                    )
                ...
                clip_rel_emb_masked = (
                    self.hparams.get("weight_2d", 0.5) * graph_rel_emb
                    + (1 - self.hparams.get("weight_2d", 0.5)) * clip_rel_emb_masked
                )
            else:
                if not self.hparams.get("load_features", None):
                    _, _, clip_rel_emb_masked = self._mask_features(
                        data_dict, None, clip_rel_emb, bidx, None, rel_count
                    )
                else:
                    clip_rel_emb_masked = clip_rel_emb
                none_mask = torch.isnan(clip_rel_emb_masked).all(dim=-1).all(dim=-1)

            predicate_prediction, relationship_prediction = (
                self.model.blip_predict_relationship(clip_rel_emb_masked, object_edges)
            )

            if self.hparams.get("mask_non_shared"):
                predicate_prediction[none_mask] = "and"

            predicate_prediction_emb = F.normalize(
                torch.from_numpy(self.rel_mapper.encode(predicate_prediction)), dim=-1
            ).cuda()
            predicate_mapping = self.rel_vocab @ predicate_prediction_emb.T

            scatter_idx = (
                torch.from_numpy(self.cust_pred2pred)
                .view(-1, 1)
                .expand(predicate_mapping.size())
                .to(predicate_mapping.device)
            )
            agg_values = (
                torch.zeros(predicate_mapping.size())
                .to(predicate_mapping.device)
                .to(predicate_mapping.dtype)
            )
            agg_values = agg_values.scatter_reduce(
                0, scatter_idx, predicate_mapping, reduce="amax"
            )
            ...
            predicate_mapping, top_predictions = (
                predicate_mapping.detach().cpu(),
                top_predictions.detach().cpu(),
            )
            ...
            objects.append(object_edges), predicates_mapped.append(
                pred_names[top_predictions.cpu()]
            ), predicates_mapped_probs.append(predicate_mapping)
    ...
    def _predict_rel_from_llava(
        ...
        for bidx in range(batch_size):
            ...
            predicate_prediction, relationship_prediction = (
                self.model.llava_predict_relationship(clip_rel_emb_masked, object_edges)
            )
            if from_distill:
                if self.hparams.get("mask_non_shared"):
                    predicate_prediction[none_mask] = "and"
    ...
    def _vis_clip_graphs(
        ...
        self._vis_graph_scannet_clip(
            data_dict["scan_id"][0],
            objects_predict[0],
            objects_gt,
            predicates[0],
            data_dict["edges"][0][: len(predicates[0])],
            data_dict["objects_id"][0],
            filename=dump_dir + f"/{data_dict['scan_id'][0]}/graph",
        )
        self._vis_graph_scannet_clip(
            data_dict["scan_id"][0],
            objects_predict[0],
            objects_gt,
            predicates_mapped[0],
            data_dict["edges"][0][: len(predicates_mapped[0])],
            data_dict["objects_id"][0],
            filename=dump_dir + f"/{data_dict['scan_id'][0]}/graph_predicates_mapped",
        )

        if self.hparams["predict_from_2d"]:
            self._vis_graph_scannet_clip(
                data_dict["scan_id"][0],
                objects_predict_clip[0],
                objects_gt,
                predicates_clip[0],
                data_dict["edges"][0][: len(predicates_clip[0])],
                data_dict["objects_id"][0],
                filename=dump_dir + f"/{data_dict['scan_id'][0]}/graph_clip",
            )
            self._vis_graph_scannet_clip(
                data_dict["scan_id"][0],
                objects_predict_clip[0],
                objects_gt,
                predicates_mapped_clip[0],
                data_dict["edges"][0][: len(predicates_mapped_clip[0])],
                data_dict["objects_id"][0],
                filename=dump_dir
                + f"/{data_dict['scan_id'][0]}/graph_predicates_mapped_clip",
            )

        if show_imgs:
            obj_imgs = data_dict["object_imgs"][0][:, 0][
                : data_dict["objects_count"][0]
            ].cpu()
            if self.hparams["blip"]:
                pred_imgs = [
                    torch.from_numpy(np.array(img) / 255).permute(2, 0, 1)
                    for img in np.array(data_dict["blip_images"][0])[:, 0][
                        : data_dict["predicate_count"][0]
                    ]
                ]
            else:

                pred_imgs = data_dict["relationship_imgs"][0][:, 0][
                    : data_dict["predicate_count"][0]
                ].cpu()
    ...
    def _vis_graph_scannet_clip(
        ...
        for index in range(len(objects)):
            ...
            dot.node(
                id,
                pred + f" [{objects_gt[index]}] " + "-" + str(object_ids[index].item()),
            )
        ...
        for i, edge in enumerate(edges):
            ...
            p_s = predicates[i]
            if np.array(
                [none_p in p_s for none_p in ["and", "unrelated", "not", "none"]]
            ).any():
    ...
    def mRecall_objects(self, hits, misses):
        ...
        if type(hits) == int:
            hits = [hits]
        total_counts = {
            class_id: (hits + misses).count(class_id) for class_id in class_ids
        }
        ...
        percent_hits = {
            k: v / total_counts[k] for k, v in hit_counts.items() if total_counts[k] > 0
        }
    ...
    def dump_stats(self, hits, misses, name):
        ...
        json.dump(
            dict(zip(class_ids, percent_hits)),
            open(os.path.join(dump_dir, f"{name}_eval_percent.json"), "w"),
        )
        json.dump(
            dict(zip(class_ids, {k: hit_counts.get(k, 0) for k in class_ids})),
            open(os.path.join(dump_dir, f"{name}_eval_hits.json"), "w"),
        )
        json.dump(
            dict(zip(class_ids, {k: miss_counts.get(k, 0) for k in class_ids})),
